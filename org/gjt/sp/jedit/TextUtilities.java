...
public class TextUtilities {
	 ...
	 * the buffer, or -1 if the bracket is unmatched (or if the character is not a
	 * bracket).
	 * 
	 * @param buffer
	 *            The buffer
	 ...
	 * @since jEdit 4.3pre15
	 */
	public static int findWordStart(CharSequence line, int pos, String noWordSep, boolean joinNonWordChars,
			boolean camelCasedWords, boolean eatWhitespace) {
	 ...
	 *            Eat only whitespace after a word, in effect this finds actual word
	 *            starts even if eating
	 * @since jEdit 4.4pre1
	 */
	public static int findWordStart(CharSequence line, int pos, String noWordSep, boolean joinNonWordChars,
			boolean camelCasedWords, boolean eatWhitespace, boolean eatOnlyAfterWord) {
		...
		for (int i = pos; i >= 0; i--) {
			...
			case WORD_CHAR:
				...
				else if (Character.isWhitespace(ch) && eatWhitespace && !eatOnlyAfterWord) {
					type = WHITESPACE;
					break;
				} else
					return i + 1; // }}}
	 ...
	 * @since jEdit 4.3pre15
	 */
	public static int findWordEnd(CharSequence line, int pos, String noWordSep, boolean joinNonWordChars,
			boolean camelCasedWords, boolean eatWhitespace) {
		if (pos != 0)
			pos--;
		...
		for (int i = pos; i < line.length(); i++) {
			...
			case WORD_CHAR:
				...
				// stop at next first upper case char if camel caseg (don't stop at every upper
				// case char)
				else if (camelCasedWords && Character.isUpperCase(ch) && !Character.isUpperCase(lastCh)) {
					return i;
				} else if (Character.isLetterOrDigit(ch) || noWordSep.indexOf(ch) != -1) {
	...
	/**
	 * Returns if the specified string is all upper case, all lower case, or title
	 * case (first letter upper case, rest lower case).
	 * 
	 * @param str
	 *            The string
	 * @since jEdit 4.4pre1
